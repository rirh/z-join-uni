	...
	<view class="wapper">
		...
		<navigator url="/pages/me/nick-name/nick-name">
			<view class="section">
				...
				<view class="contant">{{ user.nickName || '' }}</view>
		...
		<view class="section" style="margin-bottom: 40rpx;" @click="handle_toggle_sex">
			...
			<view class="contant">{{ user.gender ? '男' : '女' }}</view>
			<image class="allow-right" src="/static/right.svg" mode="scaleToFill"></image>
		</view>

		<uni-section title="附加信息" type="line"></uni-section>
		<view class="section">
			<view class="label">注册IP</view>
			<view class="contant" style="color: #666;margin-right:20rpx ;">{{ user.register_ip }}</view>
		</view>
		<view class="section">
			<view class="label">注册时间</view>
			<view class="contant" style="color: #666;margin-right:20rpx ;">{{ moment(user.register_date).format('YYYY-MM-DD hh:mm') }}</view>
		</view>
		<view class="section" style="margin-bottom: 40rpx;">
			<view class="label">系统语言</view>
		...
		<navigator url="/pages/me/signature/signature">
			<view class="section"><view class="label">这个人很懒什么也没有留下</view></view>
		</navigator>
	...
	methods: {
		...
		handle_toggle_sex() {
			uni.showActionSheet({
				...
				success: async res => {
					uni.showLoading();
					const { code } = await this.$store.dispatch('auth/update', {
