<template>
	<view>
		...
		<view class="" v-if="integral <= 0">
			<!-- #ifdef MP-WEIXIN -->
			<button type="default" open-type="share" :disabled="integral > 0" @click="handle_buy">
				获得🥕
			</button>
			<!-- #endif -->
			<!-- #ifndef MP-WEIXIN -->
		...
		<view class="">
			<button type="default" :disabled="model === 3 || integral <= 0" @click="handle_do_3">
				小兔子🐰舒缓の服务
			</button>
		</view>
		<view class="">
			<button type="default" :disabled="model === 4 || integral <= 0" @click="handle_do_4">
				小兔子🐰活力の服务
			</button>
		</view>

		<view class="">
			<button type="default" :disabled="model === 1 || integral <= 0" @click="handle_do_1">
				小狐狸🦊安神の服务
			</button>
		</view>
		<view class="">
			<button type="default" :disabled="model === 2 || integral <= 0" @click="handle_do_2">
				小狐狸🦊禅式の服务
			</button>
		</view>
		<view class="">
			<button type="default" :disabled="model === 7 || integral <= 0" @click="handle_do_7">
				小公主👸芳香の服务
			</button>
		</view>
		<view class="">
			<button type="default" :disabled="model === 8 || integral <= 0" @click="handle_do_8">
				小公主👸尊贵の服务
			</button>
		</view>

		<view class="">
			<button type="default" :disabled="model === 5 || integral <= 0" @click="handle_do_5">
				小狐狸🦊、小兔子🐰和小公主👸一起专享の服务
			</button>
		</view>
		<view class="">
			<button type="default" :disabled="model === 6 || integral <= 0" @click="handle_do_6">
				小狐狸🦊、小兔子🐰和小公主👸一起帝王の服务
			</button>
		</view>
		<view class="">
			<button type="default" :disabled="iswork === false" @click="handle_stop">
				贤者🏖️时间
			</button>
		</view>
		<view class="" v-if="dev_open">
			<view>
				<slider @change="(e) => (duration = e.detail.value)" show-value :max="10000" :mim="1" />
			</view>
			<view class="uni-list">
			...
			<view class="uni-list">
				<radio-group @change="e=>model=e.target.value">
					<label class="uni-list-cell uni-list-cell-pd" v-for="(item, index) in 8" :key="item.value">
						<view>
